...
def train(args):
    ...
    def callback_Adv(epoch, iteration, model, adversarial_model):
        ...
        if iteration % n_batches == 0:
            ...
            for i in range(len(X_valid) // args.batch_size):
                idx = slice(offset, offset+args.batch_size)
                Xt, yt, Zt = X_train[idx], y_train[idx], Z_train[idx]
                X_var = wrap_X(Xt); y_var = wrap(yt)
                Z_var = Variable(torch.squeeze(torch.from_numpy(Zt)))
                y_pred_1 = model(X_var)
                t1 = unwrap(F.nll_loss(adversarial_model(y_pred_1), Z_var));train_loss.append(t1)
                #tl = unwrap(loss(y_pred_1, y_var))
                X = unwrap_X(X_var); y = unwrap(y_var)

                Xv, yv, Zv = X_valid[idx], y_valid[idx], Z_train[idx]
                X_var = wrap_X(Xv); y_var = wrap(yv)
                Z_var = Variable(torch.squeeze(torch.from_numpy(Zv)))
                y_pred = model(X_var)
                v1 = unwrap(F.nll_loss(adversarial_model(y_pred), Z_var));valid_loss.append(v1)
                #vl = unwrap(loss(y_pred, y_var)); valid_loss.append(vl)
                Xv = unwrap_X(X_var); yv = unwrap(y_var); y_pred = unwrap(y_pred)
                yy.append(yv); yy_pred.append(y_pred)
    ...
    '''----------------------------------------------------------------------- '''    
    logging.warning("Pre-Training Classifier...")
    iteration=1
    ...
    logging.warning("Pre-Training Classifier done...")
    eh.save(model, settings)
    '''----------------------------------------------------------------------- ''' 
    ''' Pre-train Advesarial '''
    '''----------------------------------------------------------------------- ''' 
    logging.warning("Pre-Training Advesarial...")
    iteration=1
    loss_rnn = []
    loss_adv = []
    ...
    for i in range(args.epochs):
        ...
        for _ in range(n_batches):
            ...
            X = unwrap_X(X_var); y = unwrap(y_var)
            callback_Adv(i, iteration, model, adversarial_model)
        t1 = time.time()
        logging.info("Epoch took {} seconds".format(t1-t0))
