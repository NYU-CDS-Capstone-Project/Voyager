...
from analysis.distribution import build_distribution
...
def main():
    ...
    for data_path in data_paths:
        ...
        for model_path in model_paths:
            ...
            y,y_pred = build_distribution(data, os.path.join(MODELS_DIR, model_path), args.batch_size)
    
    ''' PLOT Distribution '''
    '''----------------------------------------------------------------------- '''
    plt.hist(y_pred[y==0], bins=50, normed=1, histtype="step", label="$p(f(X)=0)$")
    plt.hist(y_pred[y==1], bins=50, normed=1, histtype="step", label="$p(f(X)=1)$")
