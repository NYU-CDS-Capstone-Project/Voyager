...
from architectures.nn_utils import BiDirectionalTreeGRU
...
class GRNNTransformSimple(nn.Module):
    ...
    def forward(self, jets):
        ...
        for i, nodes in enumerate(levels[::-1]):
            ...
            u_k = self.activation(self.conv(self.activation(self.conv(self.activation(self.conv(u_k.view(1,1,shape_u_k[0], shape_u_k[1]))))))).view(shape_u_k[0], shape_u_k[1])
            
            #logging.info('After', u_k.size())
            ...
            if len(inner) > 0:
                ...
                shape_h = h.size()
                h = self.activation(self.conv(self.activation(self.conv(self.activation(self.conv(h.view(1,1,shape_h[0],shape_h[1]))))))).view(shape_h[0], shape_h[1])    
                #h = self.activation(h)
