...
args.DATA_DIR = '/scratch/psn240/capstone/data/w-vs-qcd/pickles/'
...
def train(args):
    ...
    def callback(iteration, model):
        ...
        if iteration % 25 == 0:
            ...
            yy, yy_pred = [], []
            logging.info("Length " + str(len(X_valid) // args.batch_size))
            ...
            logging.info("Here 12")
            model.train()
            logging.info("Here 13")
        return out_str
    ...
    for i in range(args.n_epochs):
        ...
        logging.info("Number of batches = %d" % n_batches)
        for j in range(n_batches):
            
            logging.info("batch# = %d" % j)
            model.train()
            optimizer.zero_grad()
